...
NN_ADDRESS=13zmHe8fgtykgbpuDV84jRqgHft46FPqbd # fill your NN address here
NN_PUBKEY=03a78ae070a5e9e935112cf7ea8293f18950f1011694ea0260799e8762c8a6f0a4 # fill your pubkey here
...
if [[ $TXFEE_SATOSHI_BYTE -gt 65 ]]; then
	echo "Fees too high now: ${TXFEE_SATOSHI_BYTE}"; exit 1
fi
...
SPLIT_VALUE=0.0001;COINAGE_TOTAL_SATOSHI=0
SPLIT_VALUE_SATOSHI=$(bc <<< "${SPLIT_VALUE}*100000000"); SPLIT_VALUE_SATOSHI=("$(printf "%.0f" ${SPLIT_VALUE_SATOSHI})")
SPLIT_TOTAL=$(bc <<< "${SPLIT_VALUE}*${SPLIT_COUNT}")
SPLIT_TOTAL_SATOSHI=$(bc <<< "${SPLIT_VALUE}*${SPLIT_COUNT}*100000000"); SPLIT_TOTAL_SATOSHI=("$(printf "%.0f" ${SPLIT_TOTAL_SATOSHI})")


if [[ $UTXOs != "null" ]]; then
	for txid in $(jq -r '.[].tx_hash_big_endian' <<<"${UTXOs}"); do txids+=("$txid"); done
	for vin in $(jq -r '.[].tx_output_n' <<<"${UTXOs}"); do vins+=("$vin"); done
	for amount in $(jq -r '.[].value' <<<"${UTXOs}"); do amounts+=("$amount"); COINAGE_TOTAL_SATOSHI=$(bc <<< "${COINAGE_TOTAL_SATOSHI}+${amount}"); done

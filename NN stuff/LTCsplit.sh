...
NN_HASH160=$(xxd -r -p <<< "${NN_PUBKEY_SHA}" | openssl dgst -rmd160 -provider legacy | sed 's/.*= //')
...
UTXOs=$(curl -s "https://api.blockcypher.com/v1/ltc/main/addrs/${NN_ADDRESS}?unspentOnly=true&includeScript=true" | jq '.txrefs|[map(select(( (.value|tonumber) != 10000)))| .[] | {tx_hash, tx_output_n, value}]')
SPLIT_VALUE=0.0001
COINAGE_TOTAL=0
...
if [[ $UTXOs != "null" ]]; then
    ...
    for amount in $(jq -r '.[].value' <<<"${UTXOs}"); do
        # Note: 'amounts' accumulation is unused; kept for compatibility but can be removed if desired
        amounts+=$(bc <<<"scale=8;${amount}*0.00000001")
    ...
    for ((tc = 0; tc <= ${TOTAL_INS} - 1; tc++)); do
        ...
        rawtx="${rawtx}${rev_txid}${vin_hex}00ffffffff" # formatted txid, which vout it was, empty scriptSig len, and sequence
    done
    ...
    for (( i=1; i<=$SPLIT_COUNT; i++ )); do
        ...
        rawtx="${rawtx}${value}2321${NN_PUBKEY}ac" # adding vout amount, script len (0x23=35 bytes), and OP codes
    done
    ...
    rawtx="${rawtx}${value}1976a914${NN_HASH160}88ac00000000" # adding change amount and OP codes and for LTC nLockTime is 0 (!)
    echo "rawtx: ${rawtx}"
    Signed=$(litecoin-cli -stdin signrawtransactionwithwallet <<<${rawtx} | jq -r '.hex')
    echo "Signed: ${Signed}"
